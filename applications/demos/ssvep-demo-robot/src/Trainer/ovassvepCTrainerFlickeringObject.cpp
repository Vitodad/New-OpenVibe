...
CTrainerFlickeringObject* CTrainerFlickeringObject::createTrainerFlickeringObject(OpenViBE::uint32 l_i32TargetId)
{
	...
	if (m_poPainter != NULL)
	{
		...
		m_poApplication->getLogManager() << OpenViBE::Kernel::LogLevel_Info << "PositionX:" << l_f32TargetX << " PositionY = " << l_f32TargetY << "\n";
...
CSSVEPFlickeringObject(NULL, ui8LitFrames, ui8DarkFrames)
{
	...
	Ogre::RealRect l_oRectangle(f32PosX - m_f32TargetWidth, f32PosY + m_f32TargetHeight, f32PosX + m_f32TargetWidth, f32PosY - m_f32TargetHeight);
	...
	m_poPointer = m_poPainter->paintTriangle(
		Point(f32PosX - 0.05f, f32PosY + m_f32TargetHeight + 0.05f),
		Point(f32PosX, f32PosY + m_f32TargetHeight - 0.05f + 0.05f),
		Point(f32PosX + 0.05f, f32PosY + m_f32TargetHeight + 0.05f),
		ColourValue(0, 1, 0));
