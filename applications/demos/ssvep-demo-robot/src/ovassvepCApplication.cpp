...
bool CApplication::setup(OpenViBE::Kernel::IKernelContext* poKernelContext)
{
	...
	m_poWindow = m_poRoot->createRenderWindow("SSVEP Stimulator", 1280, 960, l_poConfigurationManager->expandAsBoolean("${SSVEP_Ogre_FullScreen}", false), &l_oOptionList);
	...
	m_poViewport->setDimensions(0.0f, 0.0f, 1.0f, 1.0f);
	m_poViewport->setBackgroundColour(Ogre::ColourValue(0.0, 0.0, 0.0));
	...
	// - Lectura del Host y Puerto del Robot Staubli del archivo: D:\openvibe-1.3.0-src\dist\share\openvibe\applications\ssvep-demo\openvibe-ssvep-demo.conf
	
	m_StaubliPort = (int)(l_poConfigurationManager->expandAsUInteger("${SSVEP_RobotStaubli_Port}"));
	m_StaubliHost = (l_poConfigurationManager->expand("${SSVEP_RobotStaubli_Host}"));
	...
	while (l_poConfigurationManager->lookUpConfigurationTokenIdentifier(l_poConfigurationManager->expand("SSVEP_Frequency_${SSVEP_FrequencyId}")) != OV_UndefinedIdentifier)
	{
		...
		(*m_poLogManager) << LogLevel_Info << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Frecuencia: " << l_f64CurrentFrequency << "\n";
